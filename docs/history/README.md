...
| `ac-7-spec.md`, `ac-7-implementation.md`, `ac-7-test-review.md` | G#7 / GH#9 — `Hotkey.from_json` now checks vocabulary as well as shape (real `kb.Key` member names, bounded char/vk, over-long chords rejected, not truncated). Merged in PR #74. Read before changing the bounds: `MAX_CHAR_LEN = 8` exists because pynput's darwin backend can report one keypress as several code points; `MAX_VK = 0x1FFFFFFF` must stay above the XF86 media-key keysyms (`0x1008FFxx`), which the PR review's suggested `0x0110FFFF` would have rejected. The first macOS CI leg failed on an unrelated sidebar test, now G#40. |
| `ac-10-spec.md`, `ac-10-implementation.md`, `ac-10-test-review.md` | G#10 / GH#12 — residue from PR #4's review. Merged in PR #76. The ticket's startup-ordering gap had already closed: since `54a3b65` moved `_timers`/`_sync_settings()`/`_drain_ui()`/`_poll_games()` into `_build_ui()`'s tail, they run before the saved hotkey arms its listener. It is documented in `__init__`, not moved (the PR review also checked every other attribute on the `toggle()`→`start()`→`loop()` path). Round 1's comment wrongly placed `_build_ui()` "up" the file, and the PR review caught a ROADMAP claim that `hotkey` was the first nested settings value (`games`/`_profile` are nested too). The macOS leg hit G#40 again. |
| `ac-18-spec.md`, `ac-18-implementation.md`, `ac-18-test-review.md` | G#18 / GH#22 — the missing `Button` click-away test in `NumBoxFocus`, merged in PR #77. The spec was written by the orchestrator, since it followed the class's existing pattern. `Button` binds `<Button-1>` on the widget, ahead of the `all` tag where `_maybe_drop_focus` lives, so a `"break"` from `Button._click` would silently keep a field focused. The test clicks the sidebar's real "Add current game" button, found by its command because the label is "+" on a collapsed rail. The sidebar is a sibling of the tab-toggled content, so it stays viewable on every tab. Round 1 missed that and built a throwaway `Button`. |
| `ac-21-spec.md`, `ac-21-design.md`, `ac-21-implementation.md`, `ac-21-test-review.md` | G#21 / GH#32 — updater review residue, merged in PR #78. `fetch_checksums` hex check; `_safe_names` docstring corrected (zipfile has stripped `..` since 3.6.2, tar was the real hole); update-check results carry `_check_seq` and land through `_apply_check`/`_apply_check_state` on the main thread, like `_poll_seq`; `Store.save()` returns a bool, and the Appearance pane shows "Couldn't save settings — changes won't be kept after closing" once. That wording doesn't claim "read-only", because any `OSError` lands there. Three rounds, both worth reading before touching this: (1) painting the notice from `_note_save` crashed opening Settings, because `_rebuild_ui()` flushes `_persist()` before the new tree exists, so never paint mid-rebuild and let `_build_ui()`'s tail paint; (2) a test calling the real `apply_hotkey()` SIGTRAPped the macOS CI runner, so stub `app.HotkeyWatcher` in any test that must reach its save. The design doc's contrast figures were corrected after merge. |
| `ac-22-spec.md`, `ac-22-design.md`, `ac-22-implementation.md`, `ac-22-test-review.md` | G#22 / GH#33 — the Minecraft sweep warning, merged in PR #80. It sits in the jitter row's hint slot, not under Interval as the ticket said: at `WINDOW_MIN_H` the Minecraft Clicking+Eating pane has ~0–1 px spare on Windows/macOS, and the jitter hint already reserves two lines. "Java sweeps may miss" is INK bold (the owner asked it to stand out; `MUTED` is ordinary secondary text here, not a caution colour), and "Java sweeps likely fail" is BAD bold. It is driven by the `"min_sweep_ms"` profile key. Never painted mid-rebuild or while Settings is open. Worth reading before any floor test: a pane check built from *allocated* child heights cannot fail, because `pack()` squeezes a card to fit; sum `winfo_reqheight()` + pack `pady` instead, and restart at a persisted scale rather than calling `_apply_ui_scale()`. The older floor tests share the blind measure (G#42). Design contrast figures were wrong twice and corrected in the doc. |
